            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="uploadForm"  method="post" enctype="multipart/form-data" target="uploadTarget">
						    <input type="hidden" name="todoid" value="${selectedTodoId}" />
				...
				<button type="button" id="approvalButton" class="btn btn-primary" onclick="requestApproval('${taskTodo.todoid}')" style="display: none;">
        결재 요청
    </button>
                </div>
            </div>
...
$(document).ready(function() {
    $('#uploadTarget').on('load', function() {
        ...
        if (response) {
            alert(response); // 파일 업로드 성공 메시지
            // 필요하다면 파일 목록을 새로고침하거나 업데이트하는 로직 추가 가능
        }
    });
});
...
function showDetails(todoid, taskName, taskStart, taskEnd, empno, status, reviewStatus, reason) {
    ...
    // 결재 상태에 따라 버튼 표시 여부 결정
    if (reviewStatus !== '결재 요청' && status === '완료' ) {
        // 조건에 맞는 경우에만 결재 요청 버튼을 표시
        $('#approvalButton').show();
    } else {
        $('#approvalButton').hide(); // 조건에 맞지 않으면 숨김
    }
    
    $('#uploadForm').attr('action', '${path}/todos/' + selectedTodoId + '/upload');
    $('#uploadForm input[name="todoid"]').val(selectedTodoId);
    // 반려 사유가 있을 경우, 모달에 표시
    ...
    success: function(files) {
        ...
        files.forEach(function(file) {
            ...
            fileListHtml += '<li><a href="${path}/todos/' + file.todoid + '/download/' + encodedFilename + '" download>' + file.filename + '</a></li>';
        });
